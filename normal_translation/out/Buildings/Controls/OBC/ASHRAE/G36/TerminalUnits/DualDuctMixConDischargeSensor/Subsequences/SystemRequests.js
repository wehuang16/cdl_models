...
module.exports = (
  ...
  const greThr13Fn = greaterthreshold_7c60ca3f({ h: 0.5*looHys, t: looHys });
  ...
  const greThr6Fn = greaterthreshold_7c60ca3f({ h: 0.5*looHys, t: looHys });
  ...
  return (
    { uAftSupCoo, uAftSupHea, VDis_flow_Set, VDis_flow, uCoo, uHea, uCooDam, uHeaDam, THeaSet, TZon, TCooSet }
  ) => {
    const thrPreResReq = thrPreResReqFn({});
    const greThr4 = greThr4Fn({ u: VDis_flow_Set });
    const greThr3 = greThr3Fn({ u: uCooDam });
    const tim3 = tim3Fn({ u: greThr3.y });
    const and5 = and5Fn({ u1: greThr4.y, u2: tim3.y });
    const gai1 = gai1Fn({ u: VDis_flow_Set });
    const greEqu = greEquFn({ u1: gai1.y, u2: VDis_flow });
