...
const controller = require("../lib/Buildings/Controls/OBC/ASHRAE/G36/TerminalUnits/ParallelFanCVF/Controller.js");
...
module.exports = async ({points, groupVariables}) => {
  ...
  const params = {
    uOpeMod: await read('uOpeMod'),
		...
		VParFan_flow: await read('VParFan_flow'),
		oveDamPos: await read('oveDamPos'),
		oveFan: await read('oveFan'),
		uHeaOff: await read('uHeaOff')
  };
  const { VAdjAreBreZon_flow, VAdjPopBreZon_flow, VMinOA_flow, VSet_flow, VZonAbsMin_flow, VZonDesMin_flow, y1Fan, yCO2, yDam, yFanStaAla, yFloSenAla, yHeaValResReq, yHotWatPlaReq, yLeaDamAla, yLeaValAla, yLowFloAla, yLowTemAla, yVal, yZonPreResReq, yZonTemResReq } = instance(params);
