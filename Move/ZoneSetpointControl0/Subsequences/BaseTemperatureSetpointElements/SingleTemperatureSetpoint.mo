...
<p>
This is a block that brings the full suite of control for a single temperature setpoint. It can be 
either a heating zone temperature setpoint or a cooling zone temperature setpoint. It contains the 
pre-cool/pre-heat mode, load-shed mode, load-rebound mode, and default mode. 
</p>
<p>
For the load shed mode specifically, there are specific elements in this logic block that 
...
of the threshold <code>delSheTho</code>, the load shed mode will be activated.
</p>
</html>"));
